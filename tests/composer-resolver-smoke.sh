...
echo json_encode([
    ...
    "options" => [
        "symlink" => false,
        "versions" => ["durable-workflow/ai" => "2.0.0-rc.9"],
    ],
], JSON_THROW_ON_ERROR);
' "$package_root")
...
composer require \
    ...
    'durable-workflow/ai:2.0.0-rc.9@RC'
...
if (($ai["version"] ?? null) !== "2.0.0-rc.9") {
    fail("Composer did not resolve durable-workflow/ai 2.0.0-rc.9.");
}
